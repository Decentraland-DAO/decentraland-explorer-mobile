...
import { Wearable } from '../../api/nft/Wearable';
...
const WearableInfo: React.FC<ContainerProps> = ({ contractAddress, tokenId, onNFTInfoLoaded }) => {

  const [item, setItem] = useState<Wearable | null>(null);
  ...
  const getData = async () => {
    ...
    try {
      ...
      setNftInfo(data);
      setItem(data.nft.data.wearable as Wearable);
  ...
  const renderGenderChip = (item: NFT) => {
    ...
    if (wearable?.bodyShapes.length === 2) {
      ...
      if (wearable?.bodyShapes[0] === "BaseMale") {
        return (<IonChip color="dark" outline={true}>
          <IonIcon icon={male}></IonIcon>
          <IonLabel>Male</IonLabel>
        </IonChip>)
      }
      else if (wearable?.bodyShapes[0] === "BaseFemale") {
        return (<IonChip color="dark" outline={true}>
        ...
        {!loading &&
          <>
            <div className={"img-container " + item?.rarity}>
              ...
              <iframe title="preview" style={{ visibility: frameLoaded ? "visible" : "hidden" }} onLoad={onFrameLoaded} src={`https://wearable-preview.decentraland.org/?contract=${contractAddress}&token=${tokenId}`}></iframe>
            </div>
            <IonCardHeader>
