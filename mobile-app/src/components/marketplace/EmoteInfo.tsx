import { IonCard, IonCardContent, IonCardHeader, IonCardSubtitle, IonCardTitle, IonChip, IonIcon, IonItem, IonLabel, IonList, IonNote, IonSegment, IonSegmentButton, IonSpinner, IonText } from '@ionic/react';
...
import { Emote } from '../../api/nft/Emote';
...
const EmoteInfo: React.FC<ContainerProps> = ({ contractAddress, tokenId, onNFTInfoLoaded }) => {

  const [item, setItem] = useState<Emote | null>(null);
  ...
  const getData = async () => {
    ...
    try {
      ...
      setNftInfo(data);
      setItem(data.nft.data.emote as Emote);
  ...
  const renderGenderChip = (item: NFT) => {
    const emote = item.data.emote;
    if (emote?.bodyShapes.length === 2) {
      ...
      if (emote?.bodyShapes[0] === "BaseMale") {
        return (<IonChip color="dark" outline={true}>
          <IonIcon icon={male}></IonIcon>
          <IonLabel>Male</IonLabel>
        </IonChip>)
      }
      else if (emote?.bodyShapes[0] === "BaseFemale") {
        return (<IonChip color="dark" outline={true}>
        ...
        {!loading &&
          <>
            <div className={"img-container " + item?.rarity}>
              ...
              <iframe title="preview" style={{ visibility: frameLoaded ? "visible" : "hidden" }} onLoad={onFrameLoaded} src={`https://wearable-preview.decentraland.org/?contract=${contractAddress}&item=1&profile=default`}></iframe>
            </div>
            <IonCardHeader>
              ...
              <IonCardSubtitle>
                ...
                {renderGenderChip(nftInfo?.nft as NFT)}
              </IonCardSubtitle>
              <IonCardSubtitle>
                {nftInfo?.order &&
                  <>
                    ...
                    <IonChip color="dark" outline={true}>
                      <IonLabel>
                        <i className={"mana " + (nftInfo?.nft.network === "ETHEREUM" ? "ethereum" : "polygon")}></i>
                      </IonLabel>
                      <IonLabel>
                        {utils.formatPrice(nftInfo?.order.price as number / (1e18))}
                      </IonLabel>
                    </IonChip>
                  </>
                }
